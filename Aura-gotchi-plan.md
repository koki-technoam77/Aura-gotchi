...
【Step 2: Gemini連携（認識・属性推測・物理振る舞い定義）】
  送信された画像をGeminiに分析させます。単なる外見だけでなく、**「ゲーム内での物理的な振る舞い（Behavior/Effects）」**も推論させます。
  Prompt: 「この手書きの絵を『ファンタジーRPGのモンスターや使い魔』として解釈してください。この絵からインスピレーションを得て、この生き物の属性（炎、水など）と特徴、さらに**『物理的な振る舞いやエフェクト（例：燃える(burns)、落ちる(falls)、跳ねる(bounces)など）』**を推論し、英語で出力してください」
...
【Step 4: Veo連携（アニメーション・動画生成）】
  ...
  → 結果: 単なる静止画ではなく、**「AIが意味を理解して物理的な動きを付与した」**アニメーションエフェクト（燃えながら浮遊するドット絵使い魔の動画ループ）がFamiliarViewに出現し、誕生する！
```
...
### 5. 使い魔ビジュアル・アニメーション (Veo Video Playback)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      → Imagenで生成した静止画をベースに、Veoが【物理エフェクト】を適用した動画を生成！
      （※ 動画生成の待ち時間十数秒間は「今、Geminiがこの絵の意味を理解し、Veoがそれに合った物理法則と命を吹き込んでいます」とアーキテクチャの凄さを解説して繋ぐ）
      → スマホに「本当に燃えて動く」使い魔が登場
...
- **Google AIスイートのフル・オーケストレーション**: Gemini Live API (リアルタイム会話と視覚認識), Computer Use (Web上でのアクション), Imagen (キャラ生成) を一つの育成ループに統合しています。
- **「これまでになさ」の証明**: 従来のAI Agentは「仕事を代わりにやらせる」だけでした。Aura-gotchiは、**「AI Agentに仕事の手順を教える（Agentic Action）過程そのものを、育成ゲームの『経験値』としてエンタメ化する」**という全く新しいパラダイムです。「タスク成功＝経験値獲得＝ペットの進化」という結びつけ方が、最もクリエイティブなAI活用法として審査員に刺さります。
