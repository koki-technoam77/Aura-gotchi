...
class GeminiSessionViewModel: ObservableObject {
  ...
  func startSession() async {
    ...
    // Wire tool call handling with game state
    toolCallRouter = ToolCallRouter(bridge: openClawBridge, gameState: gameState)
    ...
    // Connect to Gemini with dynamic system instruction (name + personality + skills injected)
    let systemPrompt = GeminiConfig.systemInstruction + gameState.dynamicPromptSuffix
    let setupOk = await geminiService.connect(systemInstruction: systemPrompt)
